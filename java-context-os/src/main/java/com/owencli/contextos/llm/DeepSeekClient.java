...
public class DeepSeekClient implements BaseLLMClient {
    ...
    public DeepSeekClient() { this(System.getenv("DEEPSEEK_API_KEY"), "deepseek-chat", DEFAULT_BASE_URL); }
    ...
    public DeepSeekClient(String apiKey, String model, String baseUrl) {
        ...
        if (this.apiKey == null || this.apiKey.isEmpty()) log.warn("DEEPSEEK_API_KEY not set");
        log.info("DeepSeekClient initialized: model={}", this.model);
    }
    ...
    public CompletableFuture<Object> complete(String prompt, String system, int maxTokens,
        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                if (response.statusCode() != 200) throw new RuntimeException("DeepSeek error: " + response.statusCode());
