...
class TaskMemory:
    ...
    async def save(self, task_type: str, intent: str, input_text: str) -> str:
        ...
        self.store.execute(
            "INSERT INTO task_records (id,user_id,task_type,intent,status,input,created_at) VALUES (?,?,?,?,?,?,?)",
    ...
    async def complete(
        ...
        self.store.execute(
            "UPDATE task_records SET status=?,output=?,error=?,token_used=?,duration_ms=?,completed_at=? WHERE id=?",
